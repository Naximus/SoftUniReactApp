...
router.get("/:id/measurements", async (req, res) => {
  ...
  try {
    const result = await measurementService.getMany(query);
    res.status(200).json(result);
  } catch (error) {
    res.status(400).json({ error: getErrorMessage(error) });
  }
...
router.get("/:id/measurements/:measurementId", async (req, res) => {
  ...
  try {
    ...
    if (result.owner.toString() !== req.user._id && req.user.type == "client") {
      // Client is trying to acces Measurement Data of another client - NOT ALLOWED
      res.status(405).json({ error: "Not allowed" });
    }
...
router.post("/:id/measurements/", async (req, res) => {
  ...
  try {
    ...
    if (!neck || neck == "") {
      throw new Error("Neck must be defined");
    }

    if (!chest || chest == "") {
      throw new Error("Chest must be defined");
    }

    if (!waistAboveTheNavel || waistAboveTheNavel == "") {
      throw new Error("Waist above the navel must be defined");
    }

    if (!waistBelowNavel || waistBelowNavel == "") {
      throw new Error("Waist below navel must be defined");
    }

    if (!hips || hips == "") {
      throw new Error("Hips must be defined");
    }

    if (!thigh || thigh == "") {
      throw new Error("Thigh must be defined");
    }
...
router.put("/:id/measurements/:measurementId/notes", async (req, res) => {
  try {
    const measurementId = req.params.measurementId;
    const { notes } = req.body;
    ...
    const result = await measurementService.updateNotes(measurementId, notes);
...
router.delete("/:id/measurements/:measurementId", async (req, res) => {
  const measurementId = req.params.measurementId;
  const owner = req.params.id;

  // TODO check if user is allowed to delete this measurement
  try {
    const result = await measurementService.delete(measurementId);
